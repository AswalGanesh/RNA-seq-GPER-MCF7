...
df <- read.csv("DGE_G1_100nM_vs_Control.csv")
...
ggplot(df_clean, aes(x = log2FoldChange, y = neg_log10_padj, color = Expression)) +
  ...
  # Labels and Limits
  labs(
    title = "Volcano Plot: G1 100nM vs Control (padj)",
    x = "log2 Fold Change",
    y = "-log10 (padj)"
  ) +
  #ylim(-0.2, 5) +
...
ggsave("D:/Project/RNA_Seq/Results/figures/volcano_100nM_vs_control.png", width = 8, height = 6, dpi = 300)
